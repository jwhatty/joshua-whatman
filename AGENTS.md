...
A bar carries two phases and the slot picks one. `--bar-delay` scatters, which is what makes a *paused* wave freeze on a frame that still reads as a waveform — that's the desktop idle state, and hover is what sets it running. `--bar-progress` marches: delay climbing with position turns the same keyframe into one crest walking the strip. Touch gets that (`@media (hover: none)`), because a hover reward that can never arrive leaves the signal frozen for the whole visit. It runs only on scenes that aren't `data-scene-state="hidden"`, and reduced motion puts the freeze back.
